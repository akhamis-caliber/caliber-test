...
const CaliberDemo = () => {
  ...
  const exportToPDF = () => {
    ...
    const moderateCount = campaignResults.filter(r => r.status === 'Moderate').length;
          ...
          <div className="wizard-actions">
            ...
            {currentStep === 7 ? (
              <button 
                ...
                className="btn-gold"
              >
                {isLoading ? 'Creating...' : 'Create Campaign & Start Scoring'}
              </button>
            ) : (
              <button 
                onClick={nextStep}
  ...
  const renderScoring = () => (
    <div className="scoring-container">
      <div className="scoring-content">
        ...
        <p>{scoringProgress}% Complete</p>
        <div className="scoring-steps">
          <div className={scoringProgress >= 20 ? 'completed' : ''}>🔍 Analyzing file structure</div>
          <div className={scoringProgress >= 40 ? 'completed' : ''}>📊 Processing data</div>
          <div className={scoringProgress >= 60 ? 'completed' : ''}>🧠 Applying AI scoring</div>
          <div className={scoringProgress >= 80 ? 'completed' : ''}>📈 Generating insights</div>
          <div className={scoringProgress >= 100 ? 'completed' : ''}>✅ Complete</div>
        </div>
      </div>
    </div>
  );
